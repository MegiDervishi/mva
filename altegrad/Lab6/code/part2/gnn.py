...
for epoch in range(epochs):
    ...
    for i in range(0, N_train, batch_size):
        ...
        adj_batch = sp.block_diag([nx.adjacency_matrix(G_train[j]) + sp.identity(G_train[j].number_of_nodes()) for j in range(i, min(N_train, i+batch_size))])
        features_batch = np.ones((adj_batch.shape[0], 1))
        
        for j in range(i, min(N_train, i+batch_size)):
            idx_batch = idx_batch + [j-i]*G_train[j].number_of_nodes()
