...
def plot_similarity_graph(X, Y, var=1.0, eps=0.0, k=5, W=None):
    """
    ...
    :param W: if not None it is the adjacency matrix of the graph  
    :return:
    """
...
def how_to_choose_epsilon(X,Y, axis=None, var=1.0):
    ...
    if axis is not None: plot_graph_matrix(X, Y, W, axis)
    return eps, W
